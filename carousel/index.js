...
//  - so if the slide is the last one (after a user has clicked through all the slides and then hits next 
// again) we must reset the 'slidePosition' to 0, in turn taking them back to the first slide! 
// 1: Info) Why reset? well, if we try access 'slides[slidePosition]' as such 'slides[3]' or 'slides[4]' 
// or 'slides[5]', these don't exist (we have 3 carousel items), so we will get an error! We have [slide0, slide1, slide2] 
// <- remember our const 'slides'? That is has an array structure in that the items index start from 0! In a none programming world, 
// we start from 1 when counting, but this is programming, so we start counting from 0 😄
...
// to our current slide! 'carousel-item-visible' is our CSS class that makes the 'carousel-item' a block element (and not hidden) and shows it!
