...
function validateAndGetFormData(form, fields) {
  ...
  for (const field of fields) {
    ...
    if (!fieldValue) {
      form[fieldValue].classList.add('error');
    }
  ...
  for (const index in activeHabbit.days) {
    ...
    element.innerHTML = `<div class="habbit__day">День ${Number(index) + 1}</div>
      ...
      <button class="habbit__delete" onclick="deleteDay(${index})">
        <img src="images/delete.svg" width="16" height="17" alt="Удалить день ${index + 1}">
      </button>`
...
/* working on habbits in popup__form */
...
function addHabbit(event) {
  ...
  const maxId = habbits.reduce((acc, habbit) => acc > habbit ? acc : habbit.id, 0);
  habbits.push({
    id: maxId + 1,
    name: data.name,
    target: data.target,
